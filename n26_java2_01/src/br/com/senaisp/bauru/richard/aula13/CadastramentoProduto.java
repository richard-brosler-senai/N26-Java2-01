...
public class CadastramentoProduto {
	...
	private static void excluirItens(Scanner sc, ArrayList<Produto> lista) {
		int id = pesquisarItem(sc, lista);
		if (id>=0) {
			Produto pr = lista.get(id);
	...
	private static void alterarItens(Scanner sc, ArrayList<Produto> lista) {
		int id = pesquisarItem(sc, lista);
		if (id>=0) {
			Produto pr = lista.get(id);
			pr.editarDados(sc);
